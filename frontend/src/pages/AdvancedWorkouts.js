...
const AdvancedWorkouts = () => {
  ...
  useEffect(() => {
    fetch('http://nattyfitness-env.eba-puanptup.us-east-2.elasticbeanstalk.com/api/workouts')
      .then((res) => res.json())
      .then((data) => {
        const advanced = data?.by_experience_level?.advanced || [];
        setWorkouts(advanced);
        setLoading(false);
      })
      .catch((err) => {
        console.error('Error fetching advanced workouts:', err);
        setLoading(false);
      });
  }, []);
  ...
  return (
    <div className="px-4 py-10">
      <h1 className="text-3xl font-bold text-center mb-6">Advanced Workouts</h1>
      {loading ? (
