...
const IntermediateWorkouts = () => {
  ...
  useEffect(() => {
    fetch('http://nattyfitness-env.eba-puanptup.us-east-2.elasticbeanstalk.com/api/workouts')
      .then((res) => res.json())
      .then((data) => {
        const intermediate = data?.by_experience_level?.intermediate || [];
        setWorkouts(intermediate);
        setLoading(false);
      })
      .catch((err) => {
        console.error('Error fetching intermediate workouts:', err);
        setLoading(false);
      });
  }, []);
  ...
  return (
    <div className="px-4 py-10">
      <h1 className="text-3xl font-bold text-center mb-6">Intermediate Workouts</h1>
      {loading ? (
